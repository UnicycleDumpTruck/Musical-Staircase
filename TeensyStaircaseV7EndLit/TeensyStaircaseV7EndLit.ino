...
int nextLit[] = {0, ledsPerStrip, (ledsPerStrip * 2), (ledsPerStrip * 3), (ledsPerStrip * 4), (ledsPerStrip * 5)};
    ...
    if(states[treadNumber] == 0) {
      if(!(PLAYCHORDS)) { 
        if (PLAYINGSONG) { 
          ...
          usbMIDI.sendNoteOn(treadNoteAssignments[treadNumber], VELOCITY, 1);
          //Serial.println(treadNoteAssignments[treadNumber]);
          needsLighting[treadNumber] = 1;
          //Serial.println(treadNumber);
        } else {
          usbMIDI.sendNoteOn(tread->notes[0], VELOCITY, (1));
        ...
        for(int i = 0; i < tread->numNotes; i++) {
          usbMIDI.sendNoteOn(tread->notes[i], VELOCITY, (1));
          //lightTread(tread->ledStart, tread->color);
          needsLighting[treadNumber] = 1;
          //Serial.println(treadNumber);
        }
  ...
  if (sinceLastLedScan > 64) {
    for(int i=0; i<numberOfTreads; i++) {
        if (needsLighting[i] == 1) {
          leds.setPixel(nextLit[i], colors[i]);
          nextLit[i]++;
          if (nextLit[i]  >  (ledStarts[i] + (ledsPerStrip-1))) {
            needsLighting[i] = 0;
          }
        } else if (needsLighting[i] == 2) {
          leds.setPixel(nextLit[i], LEDOFF);
          nextLit[i]--;
          if (nextLit[i]  <  (ledStarts[i])) {
            needsLighting[i] = 2;
